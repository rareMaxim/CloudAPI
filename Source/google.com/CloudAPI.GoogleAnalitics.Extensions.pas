...
type
  ...
  protected
    procedure RaiseNotAssignedData(const HitType: TgaTypeTreatment; const
      AProperty: string);
  public
    procedure FillData(const HitType: TgaTypeTreatment; ADataStorage:
...
procedure TgaUser.FillData(const HitType: TgaTypeTreatment; ADataStorage: TStringList);
begin
...
procedure TgaSystem.FillData(const HitType: TgaTypeTreatment; ADataStorage: TStringList);
begin
...
procedure TgaGeneral.FillData(const HitType: TgaTypeTreatment; ADataStorage: TStringList);
begin
...
procedure TgaSession.FillData(const HitType: TgaTypeTreatment; ADataStorage: TStringList);
begin
...
procedure TgaContentInformation.FillData(const HitType: TgaTypeTreatment;
  ADataStorage: TStringList);
begin
  ...
  else if HitType = TgaTypeTreatment.screenview then
    RaiseNotAssignedData(HitType, 'ScreenName');
...
procedure TgaException.FillData(const HitType: TgaTypeTreatment; ADataStorage:
  TStringList);
begin
  if (not Text.IsEmpty) and (HitType = TgaTypeTreatment.Exception) then
    ADataStorage.AddPair('exd', Text).AddPair('exf', IsFatal.ToInteger.ToString);
end;

{ TgaTrafficSources }

procedure TgaTrafficSources.FillData(const HitType: TgaTypeTreatment;
  ADataStorage: TStringList);
begin
...
procedure TgaHit.FillData(const HitType: TgaTypeTreatment; ADataStorage: TStringList);
begin
...
procedure TgaAppTracking.FillData(const HitType: TgaTypeTreatment; ADataStorage:
  TStringList);
begin
...
procedure TgaExtension.RaiseNotAssignedData(const HitType: TgaTypeTreatment;
  const AProperty: string);
begin
  raise Exception.CreateFmt('For method %s you must set property %s', [HitType.ToString,
    AProperty]);
end;
